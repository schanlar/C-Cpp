...
typedef struct list_properties{
    ...
    unsigned int length;    // Number of active elements in the list
    unsigned int capacity;  // Total capacity of the array
    unsigned int deletions; // Number of deleted elements
    unsigned int occupied;  // Number of occupied slots (active + deleted)
}LIST;
...
void status(const LIST* pl){
    ...
    printf("LENGTH: %d; CAPACITY: %d; DELETIONS: %d; OCCUPIED: %d\n", pl->length, 
...
void display(const LIST* pl){
    ...
    printf("[");
    for (unsigned int i=0; i<pl->occupied; i++){
        if (!isnan(pl->array[i])){
            if (print_count > 0){
                printf(", ");
            }
...
void append(LIST* pl, double num){
    ...
    pl->length++;
    pl->occupied = pl->length + pl->deletions;
}
 ...
 * Within this context, two lists are equal if they have the same active
...
int is_equal(const LIST* pl1, const LIST* pl2){
    ...
    unsigned int last_idx2 = 0;
    for (unsigned int idx1=0; idx1<pl1->occupied; idx1++){
        if (isnan(pl1->array[idx1])) continue;
        double num1 = pl1->array[idx1];
        for (unsigned int idx2=last_idx2; idx2<pl2->occupied; idx2++){
 ...
 * This function first compactifies the list to remove all NaN values,
 * then uses the quicksort algorithm from the standard library to sort the
 * remaining elements.
 */
void sort(LIST* pl){
    if (!list_exists(pl)) return;
